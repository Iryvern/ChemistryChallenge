...
pygame.display.set_caption("Click to color")

# Set up the clock
clock = pygame.time.Clock()

# Load the background image
background_image = pygame.image.load("colored_image.png").convert()
...
def pickcolor_button_action():
    global pick_color_mode
    pick_color_mode = True

pickcolor_button_text = "PickColor"
pickcolor_button_color = (255, 255, 0)  # RGB color for yellow
pickcolor_button = Button(10, 100, 120, 50, pickcolor_button_text, button_font, (0, 0, 0), pickcolor_button_color, lambda: pickcolor_button_action())
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # If pick_color_mode is active, get the color of the clicked pixel and update the color picker
            if pick_color_mode:
                # Get the color of the pixel at the mouse position
                color = pygame.display.get_surface().get_at(mouse_pos)
                h, _, _, _ = pygame.Color(*color).hsla
                cp.p = h / 360  # set the color picker position
                pick_color_mode = False  # deactivate pick_color_mode
                continue  # Skip other actions on this click
            
            # If the click was not inside the ColorPicker or the Button, perform the color change
            if not cp.rect.collidepoint(mouse_pos) and not button.rect.collidepoint(mouse_pos) and not pickcolor_button.rect.collidepoint(mouse_pos):
                # Get the color of the pixel at the mouse position
        ...
        pickcolor_button.handle_event(event)
